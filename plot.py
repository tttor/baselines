...
def plot_opt_data(mode, data, log_dir):
    ...
    plt.savefig(os.path.join(log_dir,mode.lower().replace(' ','_')+'_openai_baselines.png'),dpi=300,format='png',bbox_inches='tight');
    plt.close(fig)
...
def plot_learning_curve(data, log_dir):
    ...
    plt.savefig(os.path.join(log_dir,'learning_curve_return_openai_baselines.png'),dpi=300,format='png',bbox_inches='tight');
    plt.close(fig)

def load_opt_data(log_dir):
    data = []; data_dict = {}
    with open(os.path.join(log_dir, 'progress.csv'), 'r') as f:
